...
function getTwitter(i) {
	...
	if(matches && $.inArray(matches[1], ['i', 'search', 'login', 'signup']) == -1) {
		...
		// Twitter Timeline
			...
			$(window).on('resize', function(){tw_cnt.css('max-height', tw_cnt.width());});
		}
	} else getPreview(i);
}
...
function getYoutube(i) {
	...
	if(matches) {
		...
		$(yt_cnt.children('img#'+yt_id)).on('load', function() {
			...
			$(window).on('resize', function(){yt_frame.css('height', yt_frame.width() * ratio);});
		});
		...
		success: function(data) {
			if(data == null || data.title == null || data.title == '') $('#'+prefix+cnt+i).parent('.'+container).remove();
				...
				if(total_images == 0) {
					...
					$('#'+prefix+imgs+i+' img').on('error', function() {$('#'+prefix+wrp+i).remove();} );
				}
